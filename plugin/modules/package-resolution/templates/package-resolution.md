...
2. **Zero-touch handled** — **Package manager setup** status line lists this PM as:
   - `already set up` → rewrite (direct) or trust PM config (indirect);
...
Use only when Decision order reached step 2 or step 5. Form the command yourself (`jf setup` config + Curation back this):

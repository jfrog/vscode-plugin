...
This procedure is reached via the soft-bridge Yes/No offer on **Cursor** and
**Claude Code**. **VS Code Copilot** does not get that offer — do not assume
Copilot already ran Consent Enable unless the user started this procedure
another way.

## 1. Ask which repository / package types to configure

This procedure is the **only** place the types question is asked — the offer
rule deliberately does not ask it. If you already asked, do not ask again; reuse
their answer.
...
**re-verifies** each key (fail-closed). Offer rules are **re-synced**, not
blindly cleared — they stay while other types remain offerable. `auto-setup`
...
hooks/rules cleanly. Routing already works in this session after `print-policy`.

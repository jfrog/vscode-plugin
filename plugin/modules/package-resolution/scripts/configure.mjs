...
//
// Invoked by the agent (absolute path baked into managed onboarding rules /
// onboarding-procedure). Mutates ~/.jfrog/agents-conf.json and syncs managed
// onboarding rules. Per-type No writes ~/.jfrog/skills-cache/apr-onboarding-v1.json;
// bare dismiss sets onboardingPrompt: "off" (global silence).
...
Commands:
  ...
  enable --repos <json>  Write enabled:true + defaultGlobalRepos; sync offer rules
  ...
  process.stdout.write(
    `${JSON.stringify({
      ...
      next: [
        ...
        "After auto-setup + sync print-policy: suggest a new chat so hooks/rules reload cleanly",
      ],
    })}\n`,
  );
}
